...
export function CardTitlePasserCommande(props) {
  ...
  return (
    <CardTitle className="card_title_passer_commande" title="Passer commande">
      <div className="phone">
        <AiFillPhone />
        <a className="!text-4xl" href={"tel:" + phone.replace(/\s+/g, "")}>
          {phone}
        </a>
      </div>
      <div className="descritpion_holder">
        {imageContact != undefined && (
          <div
            className={
              props.showOpening ? "image_holder_square" : "image_holder_wide"
            }
          >
        ...
        {props.showOpening && (
          <div className="opening">
            <div className="title_holder ">
              <h1 className="!text-4xl !p-4">Ouverture</h1>
            </div>
            {openingTime.map((value, index) => {
              return (
                <div key={value.id} className="opening_holder">
                  <p className="!text-4xl">{value.attributes.Jour + " "}</p>{" "}
                  <p>:</p>{" "}
                  <p className="!text-4xl">
                    {" " + value.attributes.Horaire_Ouverture} -{" "}
                    {value.attributes.Horaire_Fermeture}
                  </p>
                </div>
              );
            })}
