...
export function Distributeur() {
    ...
    [window.loadingDistributeurs, window.errorDistributeurs, window.dataDistributeurs] = useFetch("api/distributeurs?populate=*", window.dataDistributeurs);
    [window.loadingDistributeurInfo, window.errorDistributeurInfo, window.dataDistributeurInfo] = useFetch("api/distributeur-info?populate=*", window.dataDistributeurInfo);

    if (window.errorEtablissement || window.errorInformation || window.errorDistributeurs) {
        return <p className="no-information">Distributeur des pizzas Oloron</p>;
    }

    if (window.loadingEtablissement || window.loadingDistributeurs || window.loadingDistributeurInfo) {
        return <Loading />;
    }
        ...
        <Transitions>
            ...
            <CardHolder>
                <CardWideImageTilteText
                    ...
                    src={(informations.attributes.Photo.data != null) ? (baseURL + informations.attributes.Photo.data.attributes.url) : null}
                    footer={(informations.attributes.Adresse != null) ? informations.attributes.Adresse : null}
                >
            ...
            <CardHolder>
                {
                    distributeurs?.map((value) => {
                        return <CardWideImageTilteText
                            ...
                            footer={(value.attributes.Prix != null) ? value.attributes.Prix : null}
                        >
                        </CardWideImageTilteText>
                    })
                }
