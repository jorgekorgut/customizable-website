...
import { DistributeurBoisson } from "pages/distributeur-boisson/distributeur-boisson";
...
function Index() {
  ...
  [window.loadingOuverture, window.errorOuverture, window.dataOuverture] =
    useFetch("api/ouvertures", window.dataOuverture);
  [window.loadingSuppliers, window.errorSuppliers, window.dataSuppliers] =
    useFetch("api/fournisseurs?populate=*", window.dataSuppliers);
  [window.loadingPizza, window.errorPizza, window.dataPizza] = useFetch(
    "api/pizzas?populate=*",
    window.dataPizza
  );
  [window.loadingParameters, window.errorParameters, window.dataParameters] =
    useFetch("api/parametre", window.dataParameters);
  ...
  let pizzaMomentUrl =
    filterPizzaDuMoment(pizzas)?.attributes.image.data.attributes.url;
  ...
  let interval =
    parameters.attributes.Temps_Pour_Changer_Photo_Pizza_Accueil_En_Secondes *
    1000;
  let transitionTime =
    parameters.attributes.Temps_Pour_Realiser_Transition_Accueil_En_Secondes *
    1000;
  ...
  return (
    <>
      <Transitions className={"transition"}>
        <NavigationBarHeader />
        <Carousel
          urlImages={pizzasMainPage}
          timeToChangeMs={interval}
          transitionTimeMs={transitionTime}
        />
        <CardHolder>
          <CardTitle title="Où sommes nous">
            <Map zoom={zoomInput} center={center}>
              <Marker position={[latInput, lngInput]} icon={pinIcon}></Marker>
            </Map>
            {deliveryRadius !== null && (
              <p>Livraison assuré dans un rayon de {deliveryRadius} km.</p>
            )}
          </CardTitle>
          <CardTitlePasserCommande
            pizzas={pizzasMainPage}
            openingTime={openingTime}
            etablissement={etablissement}
            showOpening={true}
            imageContact={pizzaMomentUrl}
          />
        </CardHolder>
        ...
        {fournisseurs.length !== 0 && (
          <>
            <div className="title_underline_holder">
              <h1 className="color_third">Produits du terroir</h1>
            </div>
            <CardHolder>
                  ...
                  <CardUnderline key={value.id} title={value.attributes.Nom}>
                    <div className="description_holder">
                      <div className="description ">
                        {value.attributes.Description_Courte}
                      </div>
                      <ImageHolderSquare
                        src={
